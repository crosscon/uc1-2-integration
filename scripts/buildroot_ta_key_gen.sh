...
gen_and_fetch_keys() {
  ...
  ssh "$target" sh -i<<EOF
# List slots
pkcs11-tool --module ${LIBTEEC_LIB} -L
# Init token
pkcs11-tool --module ${LIBTEEC_LIB} --init-token --slot ${slot} --label "${prefix_c}Token" --so-pin 1234
# Init pin
pkcs11-tool --module ${LIBTEEC_LIB} --slot ${slot} --login --so-pin 1234 --init-pin --pin 1234
# Generate key pair
pkcs11-tool --module ${LIBTEEC_LIB} \
